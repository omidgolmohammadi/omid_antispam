...
local function list_all_plugins(only_enabled)
  ...
  local text = text..'\nThere are '..nsum..' پلاگین های نصب شده.\n'..nact..' لاگین های فعال و'..nsum-nact..' غیر فعال'
  return text
end
...
local function list_plugins(only_enabled)
  ...
  local text = text..'\n'..nact..' پلاگین های فعال '..nsum..' پلاگین های نصب شده'
  return text
end
...
local function reenable_plugin_on_chat(receiver, plugin)
  ...
  if not _config.disabled_plugin_on_chat[receiver] then
    return 'هیچ پلاگین غیر فعال مختص این گروه وجود ندارد'
  end
...
local function run(msg, matches)
  ...
  if matches[1] == 'disable' and is_sudo(msg) then --after changed to moderator mode, set only sudo
    if matches[2] == 'plugins' then
    	return 'این پلاگین قابل غیر فعال شدن نیست'
    end
